...
module.exports = (bus, client, transform) => {
	return args => {
		...
		const params = Object.assign({albumId}, creds);
		return client.getAlbum(params)
			.then(album => {
				if (album) {
					collection = Object.assign({}, collection, transform(spec, album));

					return client.getVideosByAlbum(Object.assign({albumId}, creds));
				}

				const error = new Error(`Album not found for id "${albumId}"`);
