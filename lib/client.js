...
class Client {
	...
	formatId(uri, channel) {
		return util.formatId(uri, channel);
	}
	...
	getAlbum(args) {
		args = args || {};
		const albumId = this.uriToId(args.albumUri);
	...
	getVideo(args) {
		args = args || {};
		const videoId = this.uriToId(args.videoUri);
	...
	static get DEFAULT_VIDEO_FIELDS() {
		return 'uri,name,description,files,pictures,duration,release_time,width,height,tags';
	}
		...
		return new Promise((resolve, reject) => {
			request(params, (err, res, body) => {
				...
				if (res.statusCode !== 200) {
					...
					return reject(new Boom(res.statusMessage, {statusCode: res.statusCode, data: body}));
				}
